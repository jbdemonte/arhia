...
function Contract() {
  ...
  self._define('replaced', 'cnt_rempla_sal_libre', function (value) {
    types.string(value);
    return charsets.commonCharset(value.substr(0, 200));
  });
  ...
  self._define('quaTitle', 'ect.ect_lib', function (value) {
    types.string(value);
    return charsets.commonCharset(value.substr(0, 100));
  });
  ...
  self._define('quaJobCode', 'ect.ect_code_insee', function (value) {
    types.string(value);
    return value.substr(0, 4);
  });
  self._define('locationName', 'ltrac.ltrac_codif_libre', function (value) {
    types.string(value);
    return charsets.addressCharset(value.substr(0, 1000));
  });

  self._define('effectiveDate', 'avt.avt_date_effet', types.dateString);
  self._define('externalAmendmentId', 'avt.avt_id_externe', types.re(/^[a-z0-9]{1,50}$/i));
...
Contract.prototype.update = function (callback) {
  ...
  // Return null value for `end` and `endReason` fields
  if (!json.cnt_fin_date || !json.mtf_code) {
    json.cnt_fin_date = null;
    json.mtf_code = null;
  }
  return Cls.arhia.run(callback, function (resolve, reject) {
