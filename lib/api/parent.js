...
Parent.prototype.export = function () {
  ...
  Object.keys(self._path).forEach(function (key) {
    ...
    var value = handler.post ? handler.post.call(self, self[key]) : self[key];
    ...
    if (handler.name in data) {
      if (data[handler.name] !== null) {
        if (arrayCaster.isArray(self[key])) {
          ...
          value = handler.import ? handler.import(data[handler.name]) : data[handler.name];
 ...
 * @param {Object|function} [options] - If function, Function used to test / cast value when set, have to return the value to be set
 * @param {function} [options.set] - function, Function used to test / cast value when set, have to return the value to be set
 * @param {function} [options.import] - Function used to cast value before import
 * @param {function} [options.post] - Function used to cast value before posted, does not modify the internal value
 * @param {function} [poster] - Function used to cast value before posted, does not modify the internal value
 * @protected
 */
Parent.prototype._define = function (name, foreignName, options, poster) {
  var value, setter;
  var self = this;
  ...
  self._path[name] = {name: foreignName, post: options.post || poster, import: options.import};
    ...
    set: function (v) {
      try {
        ...
        if (this.constructor.Error) {
          throw new this.constructor.Error(name + ": " + e.message);
        } else {
          throw e;
        }
      }
